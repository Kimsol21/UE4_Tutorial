...
AFountain::AFountain()
{
	...
	PrimaryActorTick.bCanEverTick = false; //Tick함수를 사용하지 않으므로 false로 변경해 성능을 높여준다. 
	...
	Splash = CreateAbstractDefaultSubobject<UParticleSystemComponent>(TEXT("SPLASH")); //컴포넌트 적용.
	Movement = CreateAbstractDefaultSubobject<URotatingMovementComponent>(TEXT("MOVEMENT"));

	RootComponent = Body;
	Water->SetupAttachment(Body); //컴포넌트들 사이의 관계 설정.
	Light->SetupAttachment(Body);
	Splash->SetupAttachment(Body);

	Water->SetRelativeLocation(FVector(0.0f, 0.0f, 135.0f)); //기본 위치 설정.
