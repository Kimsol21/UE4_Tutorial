...
// End Cross Module References
	...
	UClass* Z_Construct_UClass_AABItemBox()
	{
		...
		if (!OuterClass)
		{
			...
			static const UE4CodeGen_Private::FMetaDataPairParam NewProp_WeaponItemClass_MetaData[] = {
				{ "Category", "Box" },
				{ "ModuleRelativePath", "ABItemBox.h" },
				{ "ToolTip", "\xc5\xac???? ?????? ?????\xcf\xb4? ?????? ?????? ?? UClass?? ?????\xcd\xb8? ?????? ???? ??????, \xc6\xaf?? \xc5\xac?????? ???\xd3\xb9??? \xc5\xac???????? ?????? ?????\xcf\xb5??? TSubclassOf \xc5\xb0???\xe5\xb8\xa6 ?????? ?? ?\xd6\xb4?." },
			};
#endif
	...
	IMPLEMENT_CLASS(AABItemBox, 318358748);
